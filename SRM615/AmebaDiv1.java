...
public class AmebaDiv1 {
  ...
  public int count(int[] X) {
    ...
    TreeSet<Integer> impossibleSize = new TreeSet<Integer>();
    for (int i = 0, n = X.length; i < n; i++) {
      if (!impossibleSize.contains(X[i])) {
        impossibleSize.add(X[i]);
        impossibleSize.remove(2 * X[i]);
      }
    }

    return impossibleSize.size();
  }
