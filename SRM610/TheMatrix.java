public class TheMatrix {
  ...
  public int MaxArea(String[] board) {
    ...
    boolean[][][] even = new boolean[height][width][width];
    for (int i = 0; i < height; i++) {
      for (int j = 0; j < width; j++)
        even[i][j][j] = ((board[i].charAt(j) == '0') ^ ((i + j) % 2 == 0));
      for (int j0 = 0; j0 < width; j0++)
        for (int j1 = j0; j1 < width && even[i][j1][j1]; j1++)
          even[i][j0][j1] = true;
    }
    boolean[][][] odd = new boolean[height][width][width];
    for (int i = 0; i < height; i++) {
      for (int j = 0; j < width; j++)
        odd[i][j][j] = ((board[i].charAt(j) == '0') ^ ((i + j) % 2 == 1));
      for (int j0 = 0; j0 < width; j0++)
        for (int j1 = j0; j1 < width && odd[i][j1][j1]; j1++)
          odd[i][j0][j1] = true;
    }
    int max = 0;
    for (int j0 = 0; j0 < width; j0++) {
      for (int j1 = 0; j1 < width; j1++) {
        for (int i0 = 0; i0 < height; i0++) {
          int i1 = i0;
          while (i1 < height && even[i1][j0][j1])
            i1++;
          if ((i1 - i0) * (j1 - j0 + 1) > max)
            max = (i1 - i0) * (j1 - j0 + 1);
          i0 = i1;
        }
      }
