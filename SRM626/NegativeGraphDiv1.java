...
public class NegativeGraphDiv1 {
  ...
  static class Graph {
    ...
    Graph(int vertices) {
      ...
      this.edges = new ArrayList<ArrayList<Edge>>(vertices);
      for (int i = 0; i < vertices; i++)
        this.edges.add(i, new ArrayList<Edge>());
    }
    ...
    void computeCost(int source) {
      cost = new long[vertices];
      ...
      while (!queue.isEmpty()) {
        ...
        for (Edge edge : edges.get(from)) {
          if (cost[edge.to] > cost[from] + edge.weight) {
            cost[edge.to] = cost[from] + edge.weight;
  ...
  public long findMin(int N, int[] from, int[] to, int[] weight, int charges) {
    ...
    for (int i = 0, E = from.length; i < E; i++) {
      graph.addEdge(0 * (N + 1) + from[i], 0 * (N + 1) + to[i], weight[i]);
      graph.addEdge(0 * (N + 1) + from[i], 1 * (N + 1) + to[i], -weight[i]);
      graph.addEdge(1 * (N + 1) + from[i], 1 * (N + 1) + to[i], weight[i]);
    }

    long[] result = new long[N + 1];
    graph.computeCost(0 * (N + 1) + 1);
    for (int i = 1; i <= N; i++)
      result[i] = graph.cost[i];

    long[][] unit = new long[N + 1][N + 1];
    for (int i = 1; i <= N; i++) {
      graph.computeCost(0 * (N + 1) + i);
      for (int j = 1; j <= N; j++) {
        unit[i][j] = Math.min(graph.cost[0 * (N + 1) + j], graph.cost[1 * (N + 1) + j]);
      }
    }
    ...
    while (charges > 0) {
      if (charges % 2 == 1) {
        ...
        for (int i = 1; i <= N; i++)
          if (previous[i] < Long.MAX_VALUE)
            for (int j = 1; j <= N; j++)
              if (unit[i][j] < Long.MAX_VALUE)
                result[j] = Math.min(result[j], previous[i] + unit[i][j]);
      }
      long[][] square = new long[N + 1][N + 1];
      for (int i = 1; i <= N; i++)
        for (int j = 1; j <= N; j++)
          square[i][j] = Long.MAX_VALUE;
      for (int i = 1; i <= N; i++)
        for (int j = 1; j <= N; j++)
          if (unit[i][j] < Long.MAX_VALUE)
            for (int k = 1; k <= N; k++)
              if (unit[j][k] < Long.MAX_VALUE)
                square[i][k] = Math.min(square[i][k], unit[i][j] + unit[j][k]);
      unit = square;
      charges /= 2;
    }
  ...
  public static void main(String[] args) {
    if (args.length == 0) {
      NegativeGraphDiv1Harness.run_test(-1);
    } else {
      for (int i=0; i<args.length; ++i)
        NegativeGraphDiv1Harness.run_test(Integer.valueOf(args[i]));
    }
  }
  // END CUT HERE
}
...
// BEGIN CUT HERE
class NegativeGraphDiv1Harness {
  ...
  static int runTestCase(int casenum__) {
    switch(casenum__) {
      case 0: {
                ...
                return verifyCase(casenum__, expected__, new NegativeGraphDiv1().findMin(N, from, to, weight, charges));
      }
      case 1: {
                int N                     = 1;
                int[] from                = {1};
      ...
      case 4: {
                ...
                long expected__           = -15328623718914L;
