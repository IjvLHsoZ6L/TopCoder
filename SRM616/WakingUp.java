...
public class WakingUp {
  ...
  public int maxSleepiness(int[] period, int[] start, int[] volume, int D) {
    ...
    int[] sumOfVolume = new int[lcm + 1];
    for (int i = 0; i < N; i++)
      for (int t = start[i]; t <= lcm; t += period[i])
        sumOfVolume[t] += volume[i];

    int[] sleepiness = new int[lcm + 1];
    sleepiness[0] = 0;
    for (int t = 1; t <= lcm; t++)
      sleepiness[t] = sleepiness[t - 1] + D - sumOfVolume[t];

    if (sleepiness[0] > sleepiness[lcm])
      return -1;

    int min = 0;
    for (int t = 1; t <= lcm; t++)
      min = Math.min(min, sleepiness[t]);
